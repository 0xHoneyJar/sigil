...
3. Generate draft recipes based on common patterns
4. Create initial `.sigilrc.yaml` files per zone
5. Create `sigil-mark/` structure
...
### sigil refine [path] "[feedback]"

Manual trigger for PR-native refinement. One-liner, no wizard.

**Behavior:**
1. Read feedback and component context
2. Infer physics adjustment
3. Apply and show diff
4. Optionally save as variant
</sigil_commands>
...
- **Right pane:** Browser via Claude MCP
- **Bottom pane:** Claude Code interaction

**Branding: Adhesion**
- Typeface: Adhesion (bundled in `assets/fonts/Adhesion-Regular.otf`)
- Colors: Monochrome — #000000 background, #FFFFFF text
- No gradients, shadows, or rounded corners
- Box-drawing characters: `┏ ┓ ┗ ┛ ━ ┃`
- The interface should feel like a precision instrument
...
<sigil_recipes>
Recipes live in `sigil-mark/recipes/{zone}/`.
...
import { useServerTick } from '@sigil/hooks';
...
**Variants:** Created when refinement produces a reusable pattern:
```
